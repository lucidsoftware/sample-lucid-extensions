...
export class OAuth2TokenService {
  // One minutes threshold to account avoid failures due to an expired access token
  ...
  /**
   * This method will fetch the given user OAuth 2.0 token information.
   * If found, it will determine if the token has not expired and is within out lifetime threshold.
   * Otherwise, it will refresh the token, store the new token inforamtion and provide the new valid access token.
